...
import { useConfirmPlacement } from "./useConfirmPlacement";
import { usePlacement } from "./usePlacement";
import { useUpdatePlacement } from "./useUpdatePlacement";
...
export function PlacementView({ studentId }: { studentId: string }) {
  const { data, isLoading, error } = usePlacement(studentId);
  const update = useUpdatePlacement(studentId);
  const confirm = useConfirmPlacement(studentId);
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              Confirm the student has arrived, enrolled, and completed their first check-in. Confirming marks the
              journey complete.
            </p>
